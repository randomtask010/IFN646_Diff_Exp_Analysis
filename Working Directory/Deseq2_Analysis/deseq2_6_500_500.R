...
# Step 2: Load Dataset
count_data <- read.table("RAW data/6_500_500.tsv", header=TRUE, row.names=1)
sample_info <- data.frame(groups = factor(rep(1:2, each=6))) # Create a sample information data frame
...
meta_data <- read.table("RAW data/6_500_500_meta.tsv", header=TRUE, row.names=1)
...
write.csv(outliers_up, "Working Directory/Output/deseq2_6_500_500_outliers_upregulated.csv", row.names = FALSE)

outliers_down <- setdiff(detected_down_DESeq2, meta_down)
write.csv(outliers_down, "Working Directory/Output/deseq2_6_500_500_outliers_downregulated.csv", row.names = FALSE)
...
write.csv(metrics_df, "Working Directory/Output/Metrics_deseq2_6_500_500.csv", row.names = FALSE)
...
outliers_up
write.csv(outliers_up, "NOISeq_3_1000_0_outliers_upregulated.csv", row.names = FALSE)
outliers_down <- setdiff(detected_down_NOISeq, meta_down)
write.csv(outliers_down, "NOISeq_3_1000_0_outliers_downregulated.csv", row.names = FALSE)
...
venn.diagram(
  ...
  output = TRUE,
  filename = "NOISeq_3_1000_0_venn_upregulated.png",
  output.type = "png",
  imagetype = "png",
  resolution = 300
)
...
if(length(detected_down_NOISeq) > 0 & length(meta_down) > 0) {
  venn.diagram(
    ...
    output = TRUE,
    filename = "NOISeq_3_1000_0_venn_downregulated.png",
    output.type = "png",
    imagetype = "png",
    resolution = 300
  )

...
# Step 2: Load Dataset
count_data <- read.table("synthetic_data/9_750_250.tsv", header=TRUE, row.names=1)
...
meta_data <- read.table("synthetic_data/9_750_250_meta.tsv", header=TRUE, row.names=1)
meta_data
...
outliers_up
write.csv(outliers_up, "NOISeq2_9_750_250_outliers_upregulated.csv", row.names = FALSE)
outliers_down <- setdiff(detected_down_NOISeq, meta_down)
write.csv(outliers_down, "NOISeq2_9_750_250_outliers_downregulated.csv", row.names = FALSE)
...
venn.diagram(
  ...
  output = TRUE,
  filename = "NOISeq2_9_750_250_venn_upregulated.png",
  output.type = "png",
  imagetype = "png",
  resolution = 300
)
...
if(length(detected_down_NOISeq) > 0 & length(meta_down) > 0) {
  venn.diagram(
    ...
    output = TRUE,
    filename = "NOISeq2_9_750_250_venn_downregulated.png",
    output.type = "png",
    imagetype = "png",
    resolution = 300
  )
